...
class px_bases_pxcommand{
	...
	protected function html_template( $content ){
		...
		$src .= '<title>'.t::h( $this->pxcommand_name[0] ).' | Pickles Framework - PX Commands</title>'."\n";

		//  fess.css をロード
		$src .= '<style type="text/css">'.$this->create_src_fess().'</style>'."\n";

		//  layout.css をロード
		$src .= '<style type="text/css">'.$this->create_src_layout_css().'</style>'."\n";

		//  jQuery をロード
		$src .= '<script type="text/javascript">'.$this->create_src_jquery().'</script>'."\n";

		$src .= '</head>'."\n";
		$src .= '<body>'."\n";
		$src .= '<div class="pxcmd-outline">'."\n";
		$src .= '<div class="pxcmd-pxfw clearfix">'."\n";
		$src .= '<div class="pxcmd-pxfw_l">'.$this->create_src_pxfw_logo_svg().' Pickles Framework (version:'.t::h($this->px->get_version()).')</div>'."\n";
		$src .= '<div class="pxcmd-pxfw_r">'."\n";
		$src .= '<form class="inline">'."\n";
		...
		$src .= '<div class="pxcmd-title">'."\n";
		if(strlen($page_title)){
			$src .= '<p class="pxcmd-category_title"><a href="?PX='.htmlspecialchars( $command_title ).'" style="color:inherit; text-decoration:none;">'.htmlspecialchars($command_title).'</a></p>'."\n";
			$src .= '<h1>'.htmlspecialchars( $page_title ).'</h1>'."\n";
		}else{
			$src .= '<h1><a href="?PX='.htmlspecialchars( $command_title ).'" style="color:inherit; text-decoration:none;">'.htmlspecialchars( $command_title ).'</a></h1>'."\n";
		}
		$src .= '</div><!-- /.pxcmd-title -->'."\n";
...
.pxcmd-title h1 {
	margin:0 0 0.3em 0;font-size:xx-large; font-weight:bold;
}
.pxcmd-title p.pxcmd-category_title {
	margin:1em 0 0 0; font-size:small; font-weight:bold;
}
.pxcmd-title h1 span{
	padding-left:20px;
}
...
<g>
	<path fill="<?php print t::h($logo_color); ?>" d="M30.088,48.687c-0.18,0.021-0.371,0.004-0.57-0.04c-0.748-0.175-1.4-0.707-2.01-1.157
