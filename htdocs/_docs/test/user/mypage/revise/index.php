...
class cont_{
	...
	public function execute(){
		return $this->page_revise_user();
	}

	/**
	 * ユーザー情報変更
	 */
	private function page_revise_user(){
		$validate = $this->page_revise_user_validate();
		...
		switch( $mode ){
			...
			case 'confirm':
				...
				break;
			case 'complete':
				return $this->page_revise_user_complete();
				break;
			default:
				$validate = array();
				if(!strlen($mode)){
					$this->px->req()->set_param('user_account', $this->px->user()->get_login_user_account());
					$this->px->req()->set_param('user_name', $this->px->user()->get_login_user_name());
					$this->px->req()->set_param('user_email', $this->px->user()->get_login_user_email());
				}
				break;
		}
		return $this->page_revise_user_input($validate);
	}
	private function page_revise_user_input($validate){
		$dao_user = $this->factory_dao_user();

		$rtn = '';
		$rtn .= '<p>'."\n";
		$rtn .= '	ユーザー登録情報を変更します。<br />'."\n";
		$rtn .= '</p>'."\n";
		...
		$rtn .= '			<td>'."\n";
		$rtn .= '				<ul class="form_elements-notes">'."\n";
		$rtn .= '					<li><strong>パスワードを変更する場合のみ</strong>入力してください。</li>'."\n";
		$rtn .= '				</ul>'."\n";
	...
	private function page_revise_user_confirm(){
		$dao_user = $this->factory_dao_user();
		...
		$rtn .= '<div class="unit form_buttons">'."\n";
		$rtn .= '	<ul>'."\n";
		$rtn .= '		<li class="form_buttons-submit"><input type="submit" value="ユーザー登録内容を変更する" /></li>'."\n";
	...
	private function page_revise_user_validate(){
		$rtn = array();
		...
		}elseif( $this->px->req()->get_param('user_account') != $this->px->user()->get_login_user_account() ){
			$dao_user = $this->factory_dao_user();
...
//		}
	...
	private function page_revise_user_execute(){
		$new_user_info = array(
		...
		if( strlen($this->px->req()->get_param('user_pw')) ){
			$new_user_info['user_pw'] = $this->px->req()->get_param('user_pw');
		}
		...
		$result = $dao_user->update_user_info( $this->px->user()->get_login_user_id(), $new_user_info );
		if( !$result ){
